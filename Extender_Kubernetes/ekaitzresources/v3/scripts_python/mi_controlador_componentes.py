...
plural_aplicaciones = "applications"
...
def conciliar_spec_status(objeto, cliente):
    ...
    eventAPI = client.CoreV1Api()
    eventObject = tipos.customResourceEventObject(action='deploying', CR_type="component",
                                                  CR_object=objeto,
                                                  message='Component deployment started.',
                                                  reason='Deploying')
    eventAPI.create_namespaced_event("default", eventObject)
    ...
    # Logramos el nombre de la aplicacion a la que pertenece
    myAppName = componente_desplegado['metadata']['labels']['applicationName']
    # Logramos el nombre original del componente
    shortName = componente_desplegado['metadata']['labels']['shortName']
    ...
    eventObject = tipos.customResourceEventObject(action='deployed', CR_type="component",
                                                  CR_object=objeto,
    ...
    field_manager = 'component-' + shortName + '-' + mi_aplicacion['metadata']['name']
    for i in range(len(mi_aplicacion['status']['components'])):
        if mi_aplicacion['status']['components'][i]['name'] == objeto['spec']['name']:
            mi_aplicacion['status']['components'][i]['status'] = "Running"
