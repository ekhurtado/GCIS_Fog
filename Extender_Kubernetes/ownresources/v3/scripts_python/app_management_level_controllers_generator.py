...
def generador():
    ...
    for i in range(App_Management_Level_Number):
        ...
        if i == App_Management_Level_Number - 1:
            generador_controlador(nombres_niveles[i], 'aplicacion')
...
def generador_controlador(Nivel_Actual, Nivel_Siguiente):
    f = open('../ficheros_despliegue/' + Nivel_Actual + '_controller_deployment.yaml', 'w')
...
def generador_CRD_tercer_nivel(Nivel_Actual):
    ...
    f = open('../CRD/' + Nivel_Actual + '_definition.yaml', 'w')
...
def generador_CRD_resto_niveles(Nivel_Actual):
    ...
    with open('../CRD/' + 'test_aux.yaml', 'r') as stream:
        ...
        aux['metadata']['name'] = Nivel_Actual + 's' + '.misrecursos.aplicacion'
        aux['spec']['names']['plural'] = Nivel_Actual + 's'
        aux['spec']['names']['singular'] = Nivel_Actual
        aux['spec']['names']['kind'] = Nivel_Actual.capitalize()
    yaml.dump(aux,f)
    aux['spec']['versions'][0]['schema']['openAPIV3Schema']['properties']['spec'] = {
        'type': 'object',
        'properties': {
