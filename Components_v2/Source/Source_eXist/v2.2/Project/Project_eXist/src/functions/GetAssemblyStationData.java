...
//			horaFin = "12:15:00";
			...
			try {
				...
				if (machineWorking == true) {
					...
					String key = System.getenv("KAFKA_KEY");	// El key siempre es el mismo -> ID de la aplicacion
					// En las aplicaciones complejas, existe mas de una salida, asi que se enviarán datos a todos los topicos
					boolean moreOutputs = true;
					int i = 1;
					while (moreOutputs = true) {
						String outputInfo = System.getenv("OUTPUT_IFMH_TOPIC_" + i);
						if (outputInfo != null) {
							String topic = outputInfo.split(";")[1];
